...
///////////////////////////////////////////////////////////////////////////////
bool PeelVolume(unsigned char* f, int L, int M, int N) {
...
#ifdef _DEBUG
  PrintElapsedTime("\tEV: removing one layer of object voxels.");
#endif
...
bool ExpandVolume(
  ...
  // padTarget and padCharacter must be different
  if(padCharacter == padTarget) return false;
      ...
      for (i = 0; i < L; i++) {
	      ...
	      if(f[idx] == padTarget) {
	        ...
	        for(ii = 0; ii < 6; ii++) {
	          ...
	          if((iidx < 0) || (iidx >= sz)) {
	            PrintInfoMessage("\n\
** Error padding the object. Volume bounds are too tight. Abort !\n");
	            exit(1);
	          }
	          if(f[iidx] == EXTERIOR) {
	            f[iidx] = padCharacter;
...
#ifdef _DEBUG
  PrintElapsedTime("EV: padding the object with one layer of extra voxels.");
#endif
	...
	int nrLayers)
{
  ...
  if(nrLayers > NR_PAD_VALUES) {
    PrintInfoMessage("\n\
Error padding object. Maximum %d layers allowed. Abort\n", 
	                   NR_PAD_VALUES);
    exit(1);
  }
