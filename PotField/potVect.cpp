...
bool CalculatePotentialField(
  ...
  double r, t;
  int v1, v2, v3;
  int startIndex, endIndex, zStartIndex, zEndIndex, yStartIndex, yEndIndex;
  //
  // check norm
  //
  if((norm != PF_NORM_L1) && (norm != PF_NORM_L2)) {
    printf("Unrecognized distance specifier: %d. Defaulting to L2 (%d).\n", 
	   norm, PF_NORM_L2);
    norm = PF_NORM_L2;
  }
  ...
  if(!CheckVolumePadding(f, L, M, N)) {
	...
	switch(norm) {
	case PF_NORM_L2:
	  printf("\t** Using L2 norm.\n");
	  break;
	case PF_NORM_L1:
	  printf("\t** Using L1 norm.\n");
	  break;
	}
	...
	for (j = 0; j < M; j++) {
		...
		for (i = 0; i < L; i++) {
			...
			for (s = startIndex; s <= endIndex; s++) {
			  ...
			  /* visibility test START - too slow
			  // printf("intersection test start...\n");
			  if(GetIndexOfBPInXYZRange(i, j, k,	
			       Bound[s].x, Bound[s].y, Bound[s].z, 
			       Bound, numBound, &v1, &v2))
			  {
			    // printf("pahse 1: check BP in range - done.\n");
			    // check if this boundary pont is visible 
			    // from the current position
			    if (IsLineCrossingBoundary(i, j, k, 
				  Bound[s].x, Bound[s].y, Bound[s].z, 
				  L, M, N, f)) 
			    {
			      // not visible
			    ...
			    // printf("pahse 2: check intersection - done.\n");
			  }
			  // printf("visiblity test - end.\n");
			   visibility test - STOP */
