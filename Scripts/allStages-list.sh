...
## and calls the allstages.sh script for every volume file that it can parse 
...
/./ {
	...
	##  we should get the path and the file name
	##
	...
	printf "\n** Executing: \n\tallStages.sh " volPath " " volFile " " volExt " " size[1] " " size[2] " " size[3] " \n\t" dc " " fs " " phd " \n\t->" od "\n";
	nrex++;
	}
	
	system("./allStages.sh " volPath " " volFile " " volExt " " size[1] " " size[2] " " size[3] " " dc " " fs " " phd " " od);
END {
	printf "********\nProcessed lines: %d.\nErrors: %d.\n********", nrex, nrer;
	printf "\nDone.\n";
}
