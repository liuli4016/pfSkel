...
// fills all the holes in the volume if there are any, and makes the volume 
// binary
//
...
// 3 values defining outside voxels
#define OUTSIDE_1 2
#define OUTSIDE_2 3
#define OUTSIDE_3 4
...
bool MakeSolidVolume(int L, int M, int N, unsigned char* vol, 
		     unsigned char backgroundVal, 
		     unsigned char objectVal,  
		     bool skipX /*= false*/,
		     bool skipY /*= false*/,
		     bool skipZ /*= false*/
		     )
{
  ...
  for(idx=0; idx < L*M*N; idx++) {
    if((vol[idx] == OUTSIDE_1) || (vol[idx] == OUTSIDE_2) || 
       (vol[idx] == OUTSIDE_3)) 
      {
